...
let ``clearEntry sets the input to "0" if it includes a decimal`` () =
  let model = {input = "1.1"; calculation = []; lastActivity = DigitInput; calculationResult = None}

  let actualModel = clearEntry model
  let expectedModel = {input = "0"; calculation = []; lastActivity = NoActivity; calculationResult = None}
